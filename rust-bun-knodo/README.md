# rust-bun-knodo: OCS + knodo-agent 集成镜像 (Rust + Bun/Svelte 变体)

`myg133/openvscode-server:rust-bun-knodo` —— 在 Rust + Bun (Svelte/SvelteKit) 工具链基础上集成 knodo 本地 Agent,
让 openvscode-server 容器变成一个可被 knodo 云端远程调度的"带 AI 的开发机"。

**适用场景**: rust 后端 + Svelte/SvelteKit 前端 (Bun runtime) 的项目开发。

## 文件结构

```
rust-bun-knodo/
├── Dockerfile                       # 基于 base-latest, 烤 5 个 entrypoint 脚本 + 装 rustup/bun
...
└── scripts/
    ...
    ├── test-knodo-integration.sh    # 8 项验证脚本
    └── extract-knodo-credentials.sh # 备份 BACKEND_WS_URL + API_TOKEN
```
...
`ovscs.pod.yaml` 里 `rust-bun-knodo-config` 这个 ConfigMap 有占位符, 改完才能 apply:
...
```bash
cd D:\MyCodes\docker\ocs\rust-bun-knodo
docker build -t myg133/openvscode-server:rust-bun-knodo .
# build 完 push 到 Docker Hub (远端 daemon 拉用)
docker push myg133/openvscode-server:rust-bun-knodo
```
